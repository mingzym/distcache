...
NAL_LISTENER_new, NAL_LISTENER_free, NAL_LISTENER_create, NAL_LISTENER_accept_block, NAL_LISTENER_accept - libnal listener functions
...
had already been added to the selector B<sel> using
NAL_LISTENER_add_to_select(), B<sel> had been subsequently selected using
NAL_SELECTOR_select(2), and there is incoming connection request waiting on
B<list>. In non-blocking applications, this function is recommended over
NAL_LISTENER_accept_block().
